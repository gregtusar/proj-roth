-- Optimized Donation-to-Voter Fuzzy Matching Algorithm
...
-- ============================================
-- STAGE 3: Exact Name + Same County (FAST)
-- ============================================
CREATE OR REPLACE TEMP TABLE matches_name_county AS
SELECT 
  d.donor_id,
  i.master_id,
  i.address_id,
  0.95 AS match_confidence,
  'exact_name_county' AS match_method
FROM donor_standardized d
INNER JOIN individual_standardized i
  ON d.first_name_clean = i.first_name_clean
  AND d.last_name_clean = i.last_name_clean
  AND d.state = i.state
  -- Same county (inferred from city/zip mapping)
  AND EXISTS (
    SELECT 1 FROM `proj-roth.voter_data.addresses` a2
    WHERE a2.city = d.city_clean 
    AND a2.zip_code = d.zip_code
    AND a2.county = i.county
  )
LEFT JOIN matches_exact e ON d.donor_id = e.donor_id
WHERE e.donor_id IS NULL; -- Not already matched

-- ============================================
-- STAGE 4: Soundex Name + Exact Location (MEDIUM)
-- ============================================
...
INNER JOIN individual_standardized i
  ...
  AND d.zip_code = i.zip_code
LEFT JOIN matches_exact e ON d.donor_id = e.donor_id
LEFT JOIN matches_name_county nc ON d.donor_id = nc.donor_id
WHERE e.donor_id IS NULL 
  AND nc.donor_id IS NULL; -- Not already matched

-- ============================================
-- STAGE 5: First Initial + Last Name + Location (SELECTIVE)
-- ============================================
...
UNION ALL
SELECT * FROM matches_name_county
UNION ALL
