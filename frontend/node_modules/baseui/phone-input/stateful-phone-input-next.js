"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.default = StatefulPhoneInputNext;
var _react = _interopRequireDefault(require("react"));
var _statefulPhoneInputContainer = _interopRequireDefault(require("./stateful-phone-input-container"));
var _phoneInputNext = _interopRequireDefault(require("./phone-input-next"));
var _defaultProps = _interopRequireDefault(require("./default-props"));
function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }
/*
Copyright (c) Uber Technologies, Inc.

This source code is licensed under the MIT license found in the
LICENSE file in the root directory of this source tree.
*/

StatefulPhoneInputNext.defaultProps = {
  ..._defaultProps.default,
  clearable: true
};
function StatefulPhoneInputNext(props) {
  return /*#__PURE__*/_react.default.createElement(_statefulPhoneInputContainer.default, props, childrenProps => /*#__PURE__*/_react.default.createElement(_phoneInputNext.default, childrenProps));
}