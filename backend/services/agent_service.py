...
async def process_message_stream(
    ...
    # Set up context - we'll manage it manually for async generators
    from core.request_context import RequestContext, UserContext, SessionContext, _request_context, _user_context
    ...
    # Set context manually to avoid issues with async generator interruption
    ctx_token = _request_context.set(request_ctx)
    user_token = _user_context.set(user_ctx)
    
    try:
        print(f"[Agent] Request context set: {request_ctx.to_dict()}")
        ...
        # Client disconnected - this is normal, don't try to reset context
        print("[Agent] Client disconnected during streaming")
        raise
    except Exception as e:
        ...
        # Clean up context tokens only if they exist and we're not in a GeneratorExit
        try:
            if ctx_token:
            ...
            # Context was already reset or in different context - ignore
            pass
